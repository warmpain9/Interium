...
const applyTradeWindowStats = () => {
	...
	const koroFindPlayer = (idOrName) => {
		...
		for (let i=0;i<koromonsLbRows.length;i++){
			...
			const ids = [p.id, p.userId, p.userID, p.pekoraId, p.pekoraID, p.pekoraUserId];
			const names = [p.name, p.username, p.userName, p.displayName];
			const idMatch = ids.some(v => v != null && String(v).trim().toLowerCase() === s);
			const nameMatch = names.some(v => v != null && String(v).trim().toLowerCase() === s);
			if (idMatch || nameMatch) return { rank:i+1, total:koromonsLbRows.length, id:p.id ?? p.userId ?? p.pekoraId, name:p.name ?? p.username, displayName:p.displayName, rap:Number(p.rap)||0, value:Number(p.value)||0, updatedAt:p.updatedAt||null };
		}
		return null;
	};
	...
	const applyKoromonsProfileBlock = () => {
		...
		const profileName = String((document.querySelector('h2[class*="username"],h2[class*="profileName"],[class*="profileName"] h2') || {}).textContent || '').trim();
		...
		(async () => {
			try {
				...
				let hit = koroFindPlayer(uid) || koroFindPlayer(profileName);
				...
				if (!hit) {
					try {
						...
						hit = koroFindPlayer(uid) || koroFindPlayer(profileName);
					} catch (_e) {}
				}
				console.info('[Interium] Koromons leaderboard lookup', { players: Array.isArray(koromonsLbRows) ? koromonsLbRows.length : 0, uid, profileName, matchedRank: hit ? hit.rank : null });
				let est = null;
